...
//		}
	...
	int pause(double pauseTime, double motorSpeed) {
			int done;
			float currentTime = AutonTimer.Get();
			if (currentTime < pauseTime) {
				DriveLeft0.Set(-motorSpeed);
				DriveLeft1.Set(-motorSpeed);
				DriveRight0.Set(motorSpeed);
				DriveRight0.Set(motorSpeed);
